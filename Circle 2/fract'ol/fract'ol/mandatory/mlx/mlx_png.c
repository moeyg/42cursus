...
int	mipng_fill_img(mlx_img_list_t *img, unsigned char *buf, png_info_t *pi)
{
  ...
  while (ipos < ilen && bpos < blen)
      ...
      if ((ipos % iline) == 0)
	{
      ...
      if (pi->depth == 16)
	bpos ++;
      if (ipos % 4 == 3 && pi->rgbor == 2)  // no alpha
...
int	mipng_verif_hdr(unsigned char *hdr, png_info_t *pi)
{
  ...
  if (pi->width <= 0 || pi->height <= 0 || (pi->depth != 8 && pi->depth != 16)
      || (pi->rgbor != 2 && pi->rgbor != 6) || compress != 0 || filter != 0 || pi->interlace != 0)
  ...
  //	 pi->width, pi->height, pi->depth, pi->rgbor, compress, filter, pi->interlace, pi->bpp);
  return (0);
}
